...
class Workspace:
    ...
    def load_graph(self, engine: Engine, kw_nodes: InputType, kw_links: InputType, kw_turns: InputType,
                   t0: float | int = 0, total_time: float | int = 60, delta_t: float | int = 15, force: bool = False):
    ...
    def load_matrices(self, engine: Engine, modes: ModeType, centroids: LabelsInput, timestamps: list[Timestamp], 
                      kws_matrices: list[dict[str, InputType]], force: bool = False):
        if self.graph is None:
            raise ValueError("Graph must be loaded before loading matrices.")
        if self.matrices is None or force:            
            matrix_loader = MatrixLoader.read(engine=engine,
                                              modes=modes, 
                                              centroids=centroids, 
                                              timestamps=timestamps,
                                              kws_matrices=kws_matrices)
