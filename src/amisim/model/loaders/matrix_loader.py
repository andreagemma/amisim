...
class MatrixLoader:
    ...
    @classmethod
    def read(cls, engine: Engine, 
             modes: ModeType, 
             centroids: LabelsInput, 
             timestamps: list[Timestamp], 
             kws_matrices: list[dict[str, InputType]]) -> "MatrixLoader":
        loader = cls()
        loader._modes = modes
        for kw_matrix in kws_matrices:
            ...
            mode = str(mode)
            loader._read(engine, kw_matrix=kw_matrix.get("matrices"), centroids=centroids, timestamps=timestamps, mode=mode)        
    ...
    def _read(self, engine: Engine, kw_matrix: InputType, centroids: LabelsInput, timestamps: list[Timestamp], mode: str = "c") -> "MatrixLoader":
